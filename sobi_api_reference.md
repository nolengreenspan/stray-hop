...
- `sobi_client.py` now also covers `rent-from-hub` (books whatever bike is at a station via `book_bike`), `report-issue`, `rfids`/`rfid-add`/`rfid-rename`/`rfid-delete`, `routes`/`route` (trip history, read-only — the API doc doesn't specify what `PATCH /routes/{id}.json` actually accepts, so that's left unimplemented), `search`, `subscriptions`, and `invoices`/`invoice`/`invoice-pay`. `areas`/`area`, `friends`, and the `users` update endpoints (`update-info`, `update-email`, `update-password`) are also covered. The password/email commands prompt via `getpass` rather than taking a flag, so secrets never land in argv or shell history; `update-email` is the only path that sets the email, even though the docs also list `email` on `update_info`, because `update_email.json` requires the account password as confirmation. Still unimplemented: `book_vehicle`/vehicle endpoints (moot — Hamilton's `has_ebikes: false`).
